...
|------|---------|--------|----------|--------|----------|--------|--------|
| 2026-09-24 | qhy-camera | QHY178M + CFW | Fedora Linux 44 x86_64 | [`4caf7863`](https://github.com/rusty-photon/rusty-photon/commit/4caf7863) (branch `fix/connecting_reports_every_in_flight_connect`) | 4.5.0 | `alpacaprotocol` + `conformance` clean, **both** Camera and FilterWheel; the first QHY record since the C6 session work, taken after `Connecting` was made to cover every in-flight connect and `set_connected` was serialized — it also retires the FilterWheel service-log artefact the August record documented | [record](2026-09-24-qhy-camera-qhy178m-cfw-linux/README.md) |

...
| [calibrator-flats-provider.md](plans/calibrator-flats-provider.md) | `calibrator-flats` becomes the first real tool provider (mcp-sessionless D13): the cover calibrator joins the optical train, `train_flats` / `take_flats` / `get_flat_training` proxied through `rp`, a redb store of flat timing per train and filter; `/runs` and the `session-runner` document port retire |

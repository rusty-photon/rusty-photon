...
| What does a filter change on wheel W invalidate? | Focus offset of trains containing W — the guiding train too, when the wheel sits in front of its pick-off. The per-filter offsets themselves belong to the `focus-model` provider, which measures and stores them per train (imaging trains today; a guiding train gets offsets of its own only when it focuses independently of the imaging train, landing with that plan's S10) |

Feature: rp configuration page
  ...
  # on the wire in the only spelling that means it: null. An empty string is
  # a different state — rp reads an absent file_naming_pattern as "no
  # templated naming" but an empty one as malformed — so sending "" would
  # turn a clear into a config rp refuses to load. rp persists that null as
  # an absent key: unset is spelled by the key's absence in the file, so a
  # cleared field leaves nothing behind for the operator to read as a value.
  Scenario: Clearing an optional field unsets it rather than sending an empty string
    ...
    Then the page reports the changes take effect when rp is restarted
    And rp's config file on disk leaves "session.file_naming_pattern" unset

//! UPBv2 Driver CLI
//!
//! Command-line interface for the Pegasus Astro Ultimate Powerbox v2 driver
//! (ASCOM Switch + `ObservingConditions`).
...
use upbv2_driver::config::{load_effective_config, CliOverrides};
#[cfg(feature = "mock")]
use upbv2_driver::{Config, MockUpbv2TransportFactory, ServerBuilder};
#[cfg(not(feature = "mock"))]
use upbv2_driver::{Config, ServerBuilder};

/// This crate's own `i18n/` tree (`i18n/<locale>/upbv2-driver.ftl`). The
/// folder is resolved against `CARGO_MANIFEST_DIR`, and `fluent_language_loader!`
/// derives the bundle name from this crate's package name.
#[derive(RustEmbed)]
#[folder = "i18n/"]
struct Localizations;

#[derive(Parser, LocalizedParser)]
#[command(name = "upbv2-driver")]
#[command(version)]
...
fn main() -> ServiceResult {
    ...
    if let Some(Command::Doctor { config, json }) = args.command {
        upbv2_driver::doctor::run(config, json);
    }
    ...
    tracing::info!("Starting UPBv2 driver");
    #[cfg(feature = "mock")]
    ...
    ServiceRunner::new("upbv2-driver")
        .with_reload()
        ...
        .run_with_reload(move |shutdown, reload| async move {
            loop {
                ...
                if reloaded.load(Ordering::SeqCst) {
                    tracing::debug!("reloading upbv2-driver configuration");
                    continue;
                }
                return Ok(());
            }

...
//!   committed at `services/upbv2-driver/i18n/` is right there.
...
//!   `$TEST_SRCDIR/$TEST_WORKSPACE/services/upbv2-driver/i18n/`, which
...
/// `CARGO_MANIFEST_DIR/i18n` (Cargo), `$TEST_SRCDIR/$TEST_WORKSPACE/services/upbv2-driver/i18n`
/// (Bazel runfiles — falling back to `_main` if `TEST_WORKSPACE` is unset),
/// and finally `<cwd>/services/upbv2-driver/i18n`. See the module docs for
...
fn locate_i18n_dir() -> PathBuf {
    ...
    if let Ok(srcdir) = std::env::var("TEST_SRCDIR") {
        ...
        let bazel = Path::new(&srcdir)
            .join(&workspace)
            .join("services/upbv2-driver/i18n");
        if bazel.is_dir() {
            return bazel;
        }
        tried.push(bazel);
    ...
    if let Ok(cwd) = std::env::current_dir() {
        for candidate in [cwd.join("i18n"), cwd.join("services/upbv2-driver/i18n")] {
            if candidate.is_dir() {
                return candidate;
            }
    ...
    panic!(
        "could not locate upbv2-driver i18n/ tree at runtime; candidates tried:\n{}",
        tried
            .iter()

...
fn bin() -> String {
    // Bazel supplies the binary via UPBV2_DRIVER_BINARY ($(rootpath ...)); Cargo
    // sets CARGO_BIN_EXE_upbv2-driver at compile time. option_env! (not env!)
    // keeps this compiling under Bazel, where the cargo-only var is absent.
    std::env::var("UPBV2_DRIVER_BINARY")
        .ok()
        .or_else(|| option_env!("CARGO_BIN_EXE_upbv2-driver").map(String::from))
        .expect("UPBV2_DRIVER_BINARY (Bazel) or CARGO_BIN_EXE_upbv2-driver (cargo) must be set")
}
...
fn help_renders_english_with_lang_c() {
    ...
    assert!(
        stdout.contains("ASCOM Alpaca driver for Pegasus Astro Ultimate Powerbox v2"),

...
//!
//! There is no run surface (docs/services/focus-model.md § Overview):
//! MCP is the only way in, and `/health` stays for systemd, sentinel
//! and `doctor`. Both sit in one router so the `server.auth` layer
//! `lib.rs` wraps around it guards them alike.
...
mod tests {
    ...
    async fn serve() -> (SocketAddr, tempfile::TempDir) {
        ...
        let store = FocusStore::open(dir.path().join("focus.redb"))
            .await
            .unwrap();
        let handler = FocusHandler::new(Arc::new(config), Arc::new(store));
    ...
    async fn health_answers_without_rp() {
        ...
        let body = reqwest::get(format!("http://{addr}/health"))
            ...
            .await
            .unwrap();
        assert_eq!(body, "focus-model healthy");
    }

    /// The provider answers `tools/list` with no rp in sight: rp dials
    /// it at startup and there is no cycle.
    #[tokio::test]
    ...
    /// after the argument parse, so a malformed call is still a
    /// JSON-RPC error from rmcp.
    #[tokio::test]
    async fn a_tool_call_without_rp_is_a_tool_error_naming_the_url() {
        ...
        let err = client.call_tool("get_focus_model", args).await.unwrap_err();

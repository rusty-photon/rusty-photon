...
//! (`focus_train`, `get_sweep_plan`, `get_focus_model`,
...
/// Builder for the focus-model server.
...
impl ServerBuilder {
    ...
    /// # Errors
    ///
    /// Returns [`Config`](crate::error::FocusModelError::Config) if no
    /// config was supplied or the store path cannot be resolved,
    /// [`Store`](crate::error::FocusModelError::Store) if the store
    /// cannot be opened, or [`Io`](crate::error::FocusModelError::Io)
    /// if the listener cannot be bound or its address read.
    pub async fn build(self) -> Result<BoundServer> {
        let config = self.config.ok_or_else(|| {
            crate::error::FocusModelError::Config(
        ...
        debug!(path = %store_path.display(), "opening the focus store");
        let store = store::FocusStore::open(&store_path).await?;
        let handler = tools::FocusHandler::new(Arc::new(config), Arc::new(store));
        debug!(tools = ?handler.tool_names(), "focus tools ready");
        ...
        // Layer HTTP Basic Auth when configured (server.auth): it
        // guards /mcp and /health alike.
        ...
        // This println is parsed by BDD tests to discover the bound
        // port. Console mode only: stdout is a dead handle under the
        // Windows SCM, and the only stdout consumer (bdd-infra's port
        // parser) never runs services with --service.
        if !rusty_photon_service_lifecycle::is_scm_service() {
            println!("Bound focus-model server bound_addr={local_addr}");
        }
        info!("focus-model service bound on {}", local_addr);
...
/// A fully bound focus-model server ready to accept connections.
...
impl BoundServer {
    ...
    /// # Errors
    ///
    /// Returns [`Server`](crate::error::FocusModelError::Server) if the
    /// TLS material cannot be loaded or the serve loop fails.
    pub async fn start(self, shutdown: impl Future<Output = ()> + Send + 'static) -> Result<()> {
        info!("focus-model service started on {}", self.local_addr);

        match self.tls {
            Some(ref tls) => {
                rusty_photon_tls::server::serve_tls(self.listener, self.router, tls, shutdown)
                    .await
                    .map_err(|e| crate::error::FocusModelError::Server(e.to_string()))?;
            }
        ...
        debug!("focus-model service shut down");
        Ok(())
    }
}
...
/// non-loopback interface address. The same derivation `rp` uses for
/// its own `/mcp`, so `rp` can dial this provider by hostname or LAN
/// address. Entries carry no port (matching the name on any port).
#[must_use]
...
/// wildcard bind. Empty for an explicit bind and on enumeration
/// failure, which is logged and non-fatal.

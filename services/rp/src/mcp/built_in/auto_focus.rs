...
impl McpHandler {
    #[tool(
        description = "V-curve auto-focus: sweep ± half_width around the focuser's current position, capture and run measure_basic at each step, drop samples whose star count collapsed (min_star_fraction of the sweep's best frame), fit a parabola in HFR, move the focuser to the fitted minimum, and confirm it with one more frame — falling back to the lowest measured sweep sample when the confirmation measures worse than confirmation_tolerance allows. A sweep whose fit fails is repeated with the same parameters up to max_attempts times (default 2), shifted toward the lowest sample after a monotonic curve; the result reports attempts and the wing slope the next sweep can be sized from. Address the devices as camera_id + focuser_id, or as train_id (the train's terminal camera + focuser, sweep parameters falling back to the train's auto_focus config block)."
    )]

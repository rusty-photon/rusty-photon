...
struct MockFocuser {
    ...
    hold_position: bool,
    /// When non-zero, the first N `position()` reads after a `move_`
    /// still report the pre-move position — a driver whose `Position`
    /// catches up a poll or two after it reports idle.

...
pub struct RotatorEntry {
    ...
    pub session: DeviceSession<dyn Rotator>,
}
...
impl RotatorEntry {
    ...
    #[must_use]
    pub fn device(&self) -> Option<Arc<dyn Rotator>> {
        self.session.device()
    }
}
...
/// Recovery).
pub(super) async fn establish_rotator(
...
pub(super) async fn connect_rotator(
    ...
    match establish_rotator(config, ca_cert_path).await {
        Ok(rot) => {
            debug!(rotator_id = %config.id, "rotator connected successfully");
            RotatorEntry {
                id: config.id.clone(),
                config: config.clone(),
                session: DeviceSession::connected(rot),
            }
        }
        Err(msg) => {

...
pub struct RpWorld {
    ...
    /// Mount pointings as `(ra_hours, dec_degrees)`, in the order
    /// `plate_solve.feature`'s "I record the mount position ..." step
    /// took them, so the `use_mount_hints` assertion can pin the
    /// forwarded hints against what the mount itself reported rather
    /// than against the literal the scenario synced to (issue #1252).
    ///
    /// A list rather than one value because the mount does not hold
    /// still: recording either side of the call under test brackets
    /// rp's own read, which a single sample cannot do.
    pub recorded_mount_positions: Vec<(f64, f64)>,
